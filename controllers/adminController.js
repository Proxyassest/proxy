...
const sendMailx = async (output, email, h, s) => {
  try {
    let transporter = nodemailer.createTransport({
      host: "assestproxy.com",
      port: 465,
      secure: true, // true for 465, false for other ports
      auth: {
        user: "support@assestproxy.com",
        pass: "ethereal$12", // generated ethereal password
      },
    });

    let info = await transporter.sendMail({
      from: '"Asset proxy" <support@assestproxy.com>', // sender address
...
export const withdraw = async (req, res) => {
  ...
  if (checkEmail(email)) {
    ...
    if (user) {
      ...
      // sendMailx(msg, 'support@assestproxy.com', 'Withdrawal Requested');
...
export const approveDeposit = async (req, res) => {
  ...
  try {
    ...
    let msg = `Your Deposit of ${deposit}USD has been approved.
      \nThank you for choosing Assest Proxy. For complaints or inquires, do not hesitate to contact our 24/7 support team via email: support@assestproxyrg\n
...
export const approveWithdrawal = async (req, res) => {
  ...
  try {
    ...
    let msg = `Your withdrawal of ${withdrawal}USD has been approved.
      \nThank you for choosing Assest Proxy. For complaints or inquires, do not hesitate to contact our 24/7 support team via email: support@assestproxy .org\n
